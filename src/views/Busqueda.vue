          ...
          <v-list>
            <v-list-item v-for="(item, index) in items" :key="index">
              ...
              <v-row class="fill-height ml-6">
                <v-col cols="12">
                  <h1>
                    $ {{ $store.getters.formatPrice(item.price) }}
                    <small style="font-size: 12px"
                      >({{ item.currency_id }})</small
                    >
                  </h1>
                </v-col>
                <v-col cols="12">
      ...
      <v-row class="fill-height" align-content="center" justify="center" v-else>
        ...
        <v-col cols="12">
          <v-row justify="center">
            <v-img src="/static/img/not_found.svg" max-width="350px"></v-img>
          </v-row>
        </v-col>
      </v-row>
    </v-container>
...
export default {
  ...
  methods: {
    ...
    async searchItems() {
      this.loading = true;
      this.items = [];
      this.items = await this.$store.getters.getItems(
